...
public class RepelBallScript : MonoBehaviour {
    ...
    void OnCollisionEnter(Collision collision)
    {
        if (collision.gameObject.GetComponent<BallScript>())
        {
            foreach (ContactPoint contact in collision.contacts)
            {
                ...
                contact.otherCollider.transform.GetComponent<Rigidbody>().AddForce(Vector3.Scale(-contact.normal, tempVect) * 2000, ForceMode.Acceleration);
            }
